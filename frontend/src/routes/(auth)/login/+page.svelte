<script lang="ts">
	...
	// Best-effort OAuth hook — if the store exposes a provider login, use it; otherwise no-op
	const loginWith = async (provider: 'google' | 'apple') => {
		try {
			...
			if (typeof anyAuth.loginWithProvider === 'function') {
				...
				console.warn(`${provider} sign-in not wired yet.`);
			}
		} catch (e) {
			console.error(e);
		}
	...
	<aside class="flex items-center justify-center bg-gray-100 px-6 py-12 sm:px-8 lg:px-12">
		<div class="w-full max-w-md">
			...
			<div class="mt-6 space-y-4">
				<button type="button" class="w-full rounded-full bg-black px-4 py-3 text-base font-medium text-white shadow hover:opacity-90 focus:outline-none" onclick={() => loginWith('google')}>
					Sign in with Google
				</button>
				<button type="button" class="w-full rounded-full bg-black px-4 py-3 text-base font-medium text-white shadow hover:opacity-90 focus:outline-none" onclick={() => loginWith('apple')}>
					Sign in with Apple
				</button>
			...
			<p class="mt-3 text-center text-sm text-gray-600">
				New User? <a href="/signup" class="font-medium underline">Create an account</a>
			</p>
			...
			{#if showEmailForm}
				<form class="mt-6 space-y-5" onsubmit={(e) => { e.preventDefault(); handleLogin(); }}>
					<!-- Email Field -->
					<div class="group">
						...
						<div class="relative">
							<input
								...
								bind:value={email}
								class="block w-full rounded-xl border-2 border-gray-300 px-3 py-3 text-gray-900 placeholder-gray-500 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
					...
					<div class="group">
						...
						<div class="relative">
							<input
								...
								disabled={isSubmitting}
								bind:value={password}
								class="block w-full rounded-xl border-2 border-gray-300 px-3 py-3 pr-12 text-gray-900 placeholder-gray-500 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
